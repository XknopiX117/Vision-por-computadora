...
### /*INICIO FUNCIONES DE PASA BAJA*\ ###
#Obtener máscara Pasa Baja
def Mask_PasaBaja(img_shift, vec, d1, d2, aux = 0):
...
def Mask_Gauss(img_shift, vec, d1, d2, aux = 0):
    ...
    if(aux == 0):
        ...
        for i in range(matriz.shape[0]):
            for j in range(matriz.shape[1]):
                dis = DistanciaOrigen((vec[1,0], vec[0,0]))
                matriz[i, j] = np.exp((-dis**2)/(2*(d1**2)))
        #Punto 2
        for i in range(matriz.shape[0]):
            for j in range(matriz.shape[1]):
                dis = DistanciaOrigen((vec[1,1], vec[0,0]))
        ...
        for i in range(matriz.shape[0]):
            for j in range(matriz.shape[1]):
                dis = DistanciaOrigen((vec[1,2], vec[0,0]))
                matriz[i, j] = np.exp((-dis**2)/(2*(d2**2)))
        #Punto 4
        for i in range(matriz.shape[0]):
            for j in range(matriz.shape[1]):
                dis = DistanciaOrigen((vec[1,3], vec[0,0]))
...
mask = Mask_PasaBaja(img_sdft, P, r1, r2)
img_mask = img_sdft * mask
img_isdft = np.fft.ifftshift(img_mask)
img_filtrada = cv.idft(img_isdft)
img_filtrada = cv.magnitude (img_filtrada[:,:, 0], img_filtrada[:,:, 1])
img_filtrada = (img_filtrada-np.amin(img_filtrada))/(np.amax(img_filtrada)-np.amin(img_filtrada))
plt.imshow(img_filtrada, cmap = 'gray')
plt.title('Imagen filtrada Pasa baja'), plt.xticks([]), plt.yticks([])
plt.show()
cv.waitKey(0)
...
r1, r2 = 7, 5
cv.namedWindow("Original")
cv.createTrackbar("Orden", "Original", 2, 10, CambiarOrdenButterworth)
cv.imshow("Original", img)
O = 2
img_dft = TransformarFourier(img)
img_sdft = np.fft.fftshift(img_dft)
...
img_dft = TransformarFourier(img)
img_sdft = np.fft.fftshift(img_dft)
mask = Mask_PasaBaja(img_sdft, P, r1, r2, 1)
img_mask = img_sdft * mask
img_isdft = np.fft.ifftshift(img_mask)
img_filtrada = cv.idft(img_isdft)
img_filtrada = cv.magnitude (img_filtrada[:,:, 0], img_filtrada[:,:, 1])
...
img_dft = TransformarFourier(img)
img_sdft = np.fft.fftshift(img_dft)
mask = Mask_PasaBaja(img_sdft, P, r1, r2, 2)
img_mask = img_sdft * mask
img_isdft = np.fft.ifftshift(img_mask)
img_filtrada = cv.idft(img_isdft)
img_filtrada = cv.magnitude (img_filtrada[:,:, 0], img_filtrada[:,:, 1])
...
img_dft = TransformarFourier(img)
img_sdft = np.fft.fftshift(img_dft)
mask = Mask_PasaBaja(img_sdft, P, r1, r2, 3)
img_mask = img_sdft * mask
img_isdft = np.fft.ifftshift(img_mask)
img_filtrada = cv.idft(img_isdft)
img_filtrada = cv.magnitude (img_filtrada[:,:, 0], img_filtrada[:,:, 1])
