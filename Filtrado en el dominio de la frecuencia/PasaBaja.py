...
img = cv.imread('RyuHayabusa.jpg', cv.IMREAD_GRAYSCALE)
...
def CambiarDistancia_PasaBaja(d):
    global D, mask
    D = d
    mask = Mask_PasaBaja(img_sdft, D)
...
cv.createTrackbar("Frecuencia", "Original", 50, 150, CambiarDistancia_PasaBaja)
...
'''
cv.namedWindow("Original")
cv.createTrackbar("Frecuencia", "Original", 49, 150, CambiarDistancia_Gauss)
cv.imshow("Original", img)
D = 50
img_dft = TransformarFourier(img)
img_sdft = np.fft.fftshift(img_dft)
